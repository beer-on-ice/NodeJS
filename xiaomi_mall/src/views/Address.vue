					...
					<div class="addr-list">
						<ul>
							<li v-for="(item,index) in addressListFilter" v-bind:class="{'check':checkIndex==index}" @click="selectAddress(index,item.addressid)">
								...
								<div class="addr-opration addr-set-default" @click="setDefault(item.addressid)">
...
export default {
	...
	methods: {
		init() {
			...
			axios({
                ...
                res.data.result.forEach((item)=>{
                    item.isDefault ? _this.selectedAddrId = item.addressid: ''
                });
			})
		},
        expand() {
        ...
        setDefault(id) {
            axios({
                method: 'POST',
                url:'/api/users/addressList/setdefault',
                data: {
                    addressId: id
                }
            }).then((res)=>{
                console.log(res);
                if(res.data.status == "0") this.init();
            })
        },
        delAddress(id) {
            this.isMdShow = true;
        ...
        sureDel() {
            ...
            axios({
                method: 'POST',
                url:'/api/users/addressList/delAddress',
                data: {
