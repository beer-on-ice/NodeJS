...
export default {
    ...
    computed: {
        ...
        totalPrice() {
            ...
            this.cartList.forEach((item)=>{
                if(item.checked == "1") {
                    money += Number(item.salePrice) * parseInt(item.productNum)
                }
            });
            return money;
        }
	...
	methods: {
        ...
        closeModal() {this.modalConfirm = false;},
        delCartConfirm(item) {
        ...
        delCart() {
            ...
            axios({
                method: 'POST',
                url: '/api/users/cart/del',
                data: {
                    'productId': _this.delItem.productId
                }
            }).then((res)=>{
        ...
        toggleCheckall() {
            ...
            axios({
                method: 'POST',
                url:'/api/users/cart/editCheckAll',
                data: {
                    checkAll: flag
                }
            }).then((res)=>{
